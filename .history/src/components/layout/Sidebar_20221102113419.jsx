import React from "react";
import { RiCloseLine } from "react-icons/ri";
import { Link } from "react-router-dom";
import { AiOutlineHistory, AiOutlineWallet } from "react-icons/ai";
import { BsFillChatDotsFill } from "react-icons/bs";
import { MdPrivacyTip, MdOutlineHighQuality, MdRateReview ,MdSettingsSuggest} from "react-icons/md";
import { BiSupport, BiTable } from "react-icons/bi";
import { GiKnightBanner } from "react-icons/gi";
import image from "../../assets/Final Direct2U Logo-01.png";
import { BiLogOutCircle } from "react-icons/bi";
import { FaUserClock } from "react-icons/fa";
import { TbDiscount } from "react-icons/tb";
import {FcFeedback} from 'react-icons/fc'


import {
  MdDashboardCustomize,
  MdOutlineProductionQuantityLimits,
  MdOutlineCategory,
  MdOutlineNotifications,
} from "react-icons/md";

const Sidebar = ({ hamb, setHamb }) => {
  const nav = [
    {
      icon: <MdDashboardCustomize className="text-xl mr-3" />,
      link: "/dashboard",
      name: "Dashboard",
    },
    {
      icon: <FcFeedback className="text-xl mr-3" />,
      link: "/feedback",
      name: "FeedBack",
    },

    // {
    //   icon: <MdOutlineCategory className="text-xl mr-3" />,
    //   link: "/categories",
    //   name: "Categories",
    // },
    // {
    //   icon: <GiKnightBanner className="text-xl mr-3" />,
    //   link: "/banners",
    //   name: "Banners",
    // },

    {
      icon: <FaUserClock className="text-xl mr-3" />,
      link: "/userkundli",
      name: "User Kundli",
    },
    {
      icon: <TbDiscount className="text-xl mr-3" />,
      link: "/discount",
      name: "Discount",
    },
    {
      icon: <BsFillChatDotsFill className="text-xl mr-3" />,
      link: "/chat",
      name: "Chat",
    },
    // {
    //   icon: <AiOutlineHistory className="text-xl mr-3" />,
    //   link: "/history",
    //   name: "History",
    // },
    {
      icon: <BiTable className="text-xl mr-3" />,
      link: "/horoscope",
      name: "Horoscope",
    },
    {
      icon: <MdOutlineHighQuality className="text-xl mr-3" />,
      link: "/specification",
      name: "Specification",
    },

    // {
    //   icon: <MdSettingsSuggest className="text-xl mr-3" />,
    //   link: "/Suggestion",
    //   name: "Suggestion ",
    // },
    // {
    //   icon: <AiOutlineWallet className="text-xl mr-3" />,
    //   link: "/wallet",
    //   name: "Wallet",
    // },
    // {
    //   icon: <MdRateReview className="text-xl mr-3" />,
    //   link: "/rating",
    //   name: "Rating / Review ",
    // },
    {
      icon: <BiLogOutCircle className="text-xl mr-3" />,
      link: "/",
      name: "Logout",
    },
  ];
  return (
    <>
      <aside className="p-4 bg-green-200">
        {/* Top */}
        <div className="w-full md:hidden relative  p-2 mb-4">
          <RiCloseLine
            onClick={() => setHamb(!hamb)}
            className="text-3xl  absolute top-2 sm:hover:rotate-[228deg] transition-transform font-bold right-2 sm:hover:text-[22px] text-[rgb(241,146,46)] cursor-pointer"
          />
        </div>
        {/* Logo */}
        <figure className="flex  flex-col items-center">
          <span className="font-bold text-[rgb(241,146,46)] text-2xl">
            Astrologers Panel
            {/* <img src={image} className="h-24" /> */}
          </span>
        </figure>
        {/* Nav-menu */}
        <nav className="py-6">
          {nav.map((nav) => {
            return (
              <Link to={nav.link} key={nav.name} className="">
                <span className="flex my-3 items-center cursor-pointer text-gray-900    tracking-wider p-2 rounded-sm">
                  {nav.icon} {nav.name}
                </span>
              </Link>
            );
          })}
        </nav>
      </aside>
    </>
  );
};

export default Sidebar;
